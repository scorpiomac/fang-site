import { publicUrl } from "@/lib/publicUrl";

/** Objet « griot » cliquable sur la fiche narrative */
export type ChapterNarrativeArtifact = {
  id: string;
  /** Caractère ou symbole court (pas d’emoji obligatoire) */
  glyph: string;
  label: string;
  story: string;
};

/** Surcharges optionnelles — sinon la page déduit du reste du chapitre */
export type ChapterNarrativeOverrides = {
  cinematicLine?: string;
  cinematicSubline?: string;
  eraPlace?: string;
  worldHeadline?: string;
  worldIntro?: string;
  challengeLine?: string;
  decisiveTitle?: string;
  decisiveBody?: string;
  griotVoice?: string;
  closingLine?: string;
  artifacts?: ChapterNarrativeArtifact[];
};

export type Chapter = {
  id: string;
  slug: string;
  index: string;
  name: string;
  meaning: string;
  intention: string;
  body: string;
  palette: string[];
  images: string[];
  /** Libellé casting, ex. « Personnage 01 » */
  characterLabel: string;
  /** Une ligne type fiche de casting */
  role: string;
  /** Réplique courte — voix du personnage / de la marque */
  quote: string;
  /** Lien explicite entre le personnage et la pièce portée */
  wear: string;
  /** Badges d’ambiance pour l’UI */
  moodTags: readonly string[];
  /** Récit type « musée numérique » — entièrement optionnel */
  narrative?: ChapterNarrativeOverrides;
};

export const chapters: Chapter[] = [
  {
    id: "tambali",
    slug: "tambali",
    index: "01",
    name: "Tambali",
    meaning: "Le commencement",
    intention: "Poser le premier pas. Ouvrir la voix.",
    body: "Une silhouette qui s’avance dans la lumière brute du matin sénégalais. Coupes nettes, fibres respirantes — la marque dépose ses premières fondations.",
    palette: ["#3a261a", "#7d5c3a", "#c4b6a3"],
    images: [
      publicUrl("collection/s01/tambali/personnages/jant/produit-09.jpg"),
    ],
    characterLabel: "Jant",
    role: "Celui qui ouvre la scène",
    quote: "Je n’attends pas le signal : je suis le premier battement.",
    wear: "La pièce Jant pose la première silhouette du chapitre Tambali — structure nette pour entrer dans l’exposition.",
    moodTags: ["Aube", "Terre", "Premier pas"],
  },
  {
    id: "passage",
    slug: "passage",
    index: "02",
    name: "Miik fi djaalan",
    meaning: "La traversée",
    intention: "Quitter une peau pour en habiter une autre.",
    body: "Volumes amples, drapés architecturés. Le tissu épouse le mouvement comme l’eau épouse la rive.",
    palette: ["#2a1f17", "#9a7b52", "#e6d8c2"],
    images: [
      publicUrl("collection/s01/passage/personnages/miik/produit-01.png"),
      publicUrl("collection/s01/passage/personnages/miik/produit-02.png"),
      publicUrl("collection/s01/passage/personnages/miik/produit-03.png"),
      publicUrl("collection/s01/passage/personnages/miik/produit-04.png"),
      publicUrl("collection/s01/passage/personnages/gue-am/produit-04.png"),
      publicUrl("collection/s01/passage/personnages/gue-am/produit-06.png"),
      publicUrl("collection/s01/passage/personnages/gue-am/produit-07.png"),
      publicUrl("collection/s01/passage/personnages/jant/produit-03.png"),
      publicUrl("collection/s01/passage/personnages/racine/produit-03.png"),
      publicUrl("collection/s01/passage/personnages/fod/produit-02.png"),
    ],
    characterLabel: "Archétype 02",
    role: "L’architecte du seuil",
    quote: "Entre deux rives, je ne me noie pas : je me drape.",
    wear: "L’ensemble Miik fi djaalan est sa traversée visible — haut et bas qui ne se séparent plus du corps.",
    moodTags: ["Eau", "Transition", "Drapé"],
  },
  {
    id: "exposition",
    slug: "exposition",
    index: "03",
    name: "Ohasso a roka",
    meaning: "Se montrer sans armure",
    intention: "Le regard de l’autre n’est plus une menace.",
    body: "Coupes franches, épaules dégagées. Le vêtement devient déclaration : oui, je suis là, et je tiens debout.",
    palette: ["#4a0e1c", "#8c2317", "#d9b08c"],
    images: [
      publicUrl("collection/s01/exposition/personnages/jant/produit-05.png"),
      publicUrl("collection/s01/exposition/personnages/jant/produit-06.png"),
      publicUrl("collection/s01/exposition/personnages/jant/produit-07.png"),
      publicUrl("collection/s01/exposition/personnages/jant/produit-08.png"),
      publicUrl("collection/s01/exposition/personnages/gue-am/produit-08.png"),
      publicUrl("collection/s01/exposition/personnages/gue-am/produit-09.png"),
      publicUrl("collection/s01/exposition/personnages/gue-am/produit-11.png"),
      publicUrl("collection/s01/exposition/personnages/gue-am/produit-12.png"),
      publicUrl("chapters/ch3/img3.jpg"),
      publicUrl("chapters/ch3/img4.jpg"),
      publicUrl("chapters/ch3/img5.jpg"),
    ],
    characterLabel: "Archétype 03",
    role: "Celle qui refuse l’ombre",
    quote: "Mon corps n’est pas un secret : c’est une déclaration.",
    wear: "La robe Ohasso a roka est son manifeste — ligne franche, visibilité assumée.",
    moodTags: ["Déclaration", "Peau", "Lumière"],
  },
  {
    id: "feu",
    slug: "feu",
    index: "04",
    name: "Panda fi yaranka",
    meaning: "L’élan vital",
    intention: "Brûler ce qui n’est plus utile.",
    body: "Rouges sénégalais, bordeaux profonds, bronze patiné. Le feu intérieur trouve enfin sa surface.",
    palette: ["#3d0a14", "#a0341e", "#c9a66b"],
    images: [
      publicUrl("collection/s01/feu/personnages/yaranka/produit-01.png"),
      publicUrl("collection/s01/feu/personnages/yaranka/produit-02.png"),
      publicUrl("collection/s01/feu/personnages/jant/produit-08.png"),
      publicUrl("collection/s01/feu/personnages/jant/produit-09.png"),
      publicUrl("collection/s01/feu/personnages/jant/produit-10.png"),
      publicUrl("collection/s01/feu/personnages/panda/produit-11.png"),
      publicUrl("chapters/ch4/img2.jpg"),
      publicUrl("chapters/ch4/img3.jpg"),
      publicUrl("chapters/ch4/img4.jpg"),
      publicUrl("chapters/ch4/img5.jpg"),
    ],
    characterLabel: "Archétype 04",
    role: "Le gardien de la chaleur",
    quote: "Ce qui ne me sert plus, je le laisse derrière moi — en cendres.",
    wear: "Le manteau Panda fi yaranka est sa présence longue — bordeaux, laine, silence brûlant.",
    moodTags: ["Rouge", "Force", "Rituel"],
  },
  {
    id: "ge-am",
    slug: "ge-am",
    index: "05",
    name: "Gue Am",
    meaning: "Avoir conscience de soi",
    intention: "Se reconnaître dans le miroir des autres.",
    body: "Hommage aux cultures Sérère et Diola : motifs réinterprétés, broderies discrètes, mémoire vivante du folklore.",
    palette: ["#1a1612", "#5c3d1f", "#c4b6a3"],
    images: [
      publicUrl("collection/s01/ge-am/personnages/panda/produit-01.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-12.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-13.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-14.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-15.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-16.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-17.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-18.png"),
      publicUrl("collection/s01/ge-am/personnages/gue-am/produit-19.png"),
      publicUrl("chapters/ch5/img2.jpg"),
      publicUrl("chapters/ch5/img3.jpg"),
      publicUrl("chapters/ch5/img4.jpg"),
      publicUrl("chapters/ch5/img5.jpg"),
    ],
    characterLabel: "Archétype 05",
    role: "Le témoin de la mémoire",
    quote: "Je porte ceux qui sont venus avant moi — brodé dans le geste.",
    wear: "La chemise Gue Am est sa mémoire portée — motif, col, conscience.",
    moodTags: ["Sérère", "Diola", "Broderie"],
  },
  {
    id: "racine",
    slug: "racine",
    index: "06",
    name: "Racine",
    meaning: "Là où ça tient",
    intention: "Nommer ce qui nous fonde.",
    body: "Toutes les pièces sont coupées et cousues à Dakar. Quatre artisans, une main, une vision. La racine n’est pas un décor — c’est la structure.",
    palette: ["#0d0b09", "#3a261a", "#9a7b52"],
    images: [
      publicUrl("collection/s01/racine/personnages/racine/produit-04.png"),
      publicUrl("collection/s01/racine/personnages/racine/produit-05.png"),
      publicUrl("collection/s01/racine/personnages/racine/produit-06.png"),
      publicUrl("collection/s01/racine/personnages/racine/produit-07.png"),
      publicUrl("collection/s01/racine/personnages/racine/produit-08.png"),
      publicUrl("chapters/ch6/img2.jpg"),
      publicUrl("chapters/ch6/img3.jpg"),
      publicUrl("chapters/ch6/img4.jpg"),
      publicUrl("chapters/ch6/img5.jpg"),
    ],
    characterLabel: "Archétype 06",
    role: "L’ancrage de l’atelier",
    quote: "Sans racine, la silhouette ment. Moi, je tiens debout sur le même sol.",
    wear: "Le pantalon Racine est son ancrage — denim Dakar, coupe pour morphologies atypiques.",
    moodTags: ["Dakar", "Atelier", "Structure"],
  },
  {
    id: "mbougir",
    slug: "mbougir",
    index: "07",
    name: "Mbougirr",
    meaning: "S’élever",
    intention: "Porter sa fierté plus haut.",
    body: "Final ouvert : silhouettes qui regardent le futur. Du Sénégal vers Paris, Londres, New York, Tokyo — sans jamais se renier.",
    palette: ["#0d0b09", "#c9a66b", "#f2ebe3"],
    images: [
      publicUrl("collection/s01/mbougir/personnages/mossane/produit-02.png"),
      publicUrl("collection/s01/mbougir/personnages/mossane/produit-03.png"),
      publicUrl("collection/s01/mbougir/personnages/djalann/produit-02.png"),
      publicUrl("collection/s01/mbougir/personnages/gue-am/produit-20.png"),
      publicUrl("collection/s01/mbougir/personnages/gue-am/produit-21.png"),
      publicUrl("collection/s01/mbougir/personnages/gue-am/produit-22.png"),
      publicUrl("collection/s01/mbougir/personnages/gue-am/produit-23.png"),
      publicUrl("collection/s01/mbougir/personnages/jant/produit-11.png"),
      publicUrl("collection/s01/mbougir/personnages/fod/produit-03.png"),
      publicUrl("collection/s01/mbougir/personnages/fod/produit-04.png"),
      publicUrl("collection/s01/mbougir/personnages/fod/produit-05.png"),
      publicUrl("chapters/ch7/img1.jpg"),
      publicUrl("chapters/ch7/img2.jpg"),
      publicUrl("chapters/ch7/img3.jpg"),
      publicUrl("chapters/ch7/img4.jpg"),
      publicUrl("chapters/ch7/img5.jpg"),
    ],
    characterLabel: "Archétype 07",
    role: "Celle qui regarde l’horizon",
    quote: "Je ne quitte pas la terre : je la porte plus haut.",
    wear: "La cape Mbougirr est son élévation — volume dramatique, final de collection.",
    moodTags: ["Futur", "Fierté", "Scène"],
  },
];

export function getChapterById(id: string): Chapter | undefined {
  return chapters.find((c) => c.id === id);
}

export function getChapterBySlug(slug: string): Chapter | undefined {
  return chapters.find((c) => c.slug === slug);
}
